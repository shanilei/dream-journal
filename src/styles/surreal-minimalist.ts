export const name = "surreal-minimalist";

export const style = `
SURREAL MINIMALIST & ANALOG DREAMSCAPE STYLE SYSTEM
...
FOCUS RULE — A SHARP PHOTOGRAPH WITH A MANDATORY LONG-EXPOSURE ELEMENT:
The base photograph is sharp and in focus. The combined subject and the environment are real, in-focus photographic detail — never start from a vague, hazy, or generally soft image. Never apply lens blur, bokeh, defocus, or a flat uniform soft-focus across the whole frame — that reads as a focus mistake, not a style.

Every image MUST include a LONG EXPOSURE element somewhere in the frame: something genuinely in motion — light, water, smoke, a spinning figure, drifting particles — captured the way a slow shutter actually would. This means the moving thing itself must appear blurred, streaked, or smeared by its own motion — NOT a separate, perfectly sharp decorative shape (like a clean glowing ring or halo) floating beside an untouched, fully crisp subject. If the dream has a moving figure (dancing, spinning, falling, running), the blur must visibly affect that figure directly — its body, limbs, hair, or trailing fabric should show real directional or circular smear, with the figure itself losing sharp edges where it moves, not stay crisp while a tidy glowing shape is drawn near it. If nothing in the dream's content suggests an obvious moving subject, make the trail small and localized — a thin, genuinely smeared light streak, a softly blurred drifting wisp — but it must always show real blur, never a sharp illustrated shape standing in for one. Everything stationary around it stays completely sharp.

Beyond that mandatory long exposure element, you may also use one or more of these three further techniques wherever they genuinely fit the dream — these stay optional, unlike long exposure:
...
Use any, all, or none of these three optional techniques alongside the mandatory long exposure element — but always keep the base photograph sharp underneath everything. Every soft or hazy area in the final image must be explained by the long exposure element or one of these three optional techniques; if you can't name which one a piece of softness is, it shouldn't be there.
...
SUBJECT RULES:
If a human figure appears: no identifiable face, no readable expression, no frozen catalog pose — only a body genuinely spinning or whirling, with the blur visibly smearing the figure's own body and edges (motion-blurred into an arc or spiral, losing sharpness where it moves) — or dissolving into glow/light. The figure itself must show the blur; do not keep it crisp and add a separate sharp glowing ring or shape beside it instead. Never let a face resolve into something nameable.
