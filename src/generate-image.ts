...
function buildPrompt(analysis: DreamAnalysis, profile: ProfileKey, styleText: string, styleName: string): string {
  ...
  const elementsInstruction =
    styleName === "risograph-collage"
      ...
      ? `ABSOLUTELY NO TEXT anywhere in the image — no letters, words, numbers, labels, or symbols of any kind, not even a single character, and not as texture, background detail, or on any object's surface (blank book covers, blank signs, numeral-free clocks, unbranded packaging — see the NO TEXT RULE above for the full list). If any listed element below would realistically carry text, render it with that surface blank instead. NO glass panels, glass frames, frosted windows, or any rectangular overlay that creates a picture-within-a-picture — the image must be a single unified scene.
