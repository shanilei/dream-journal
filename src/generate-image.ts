...
function buildPrompt(analysis: DreamAnalysis, profile: ProfileKey, styleText: string, styleName: string): string {
  ...
  const elementsInstruction =
    styleName === "risograph-collage"
      ...
      ? `Identify up to the 3 most central, dream-defining elements from these dream symbols/locations: ${elements || analysis.themes?.join(", ") || "the dream's central image"}. Render each one literally and sharply, with real visual detail and texture — recognizable on close inspection, not vague or simplified. Then weave them together into ONE single, unexpected composition: let them overlap, nest inside one another, or merge at a shared edge, rather than floating as separate scattered objects — the result should read as one coherent surreal form, interesting and a little hard to fully untangle at first glance, even though every individual piece is sharp and accurate. Place this combined form in a magical-realism context — dramatically scaled up or down, or set somewhere it would not normally belong — against the flat color-blocked backdrop. ALWAYS include a long-exposure light or motion trail somewhere in the frame — if nothing in this dream's content suggests an obvious moving subject, make it small and localized (a thin trailing light streak, a soft drifting wisp) rather than skipping it. You may also, optionally, layer in abstract motion blur, a frosted-glass surface, or Orton-effect glow wherever they genuinely fit this dream — for example a spinning figure motion-blurred into an arc, or a glass surface diffusing part of the frame. The rest of the combined form and the backdrop stay pin-sharp; do not apply any blanket lens blur, bokeh, or soft focus across the whole image.`
